...
// PocketBase always needs a directory on disk, so this is folio2'
// equivalent of SQLite's ":memory:" mode.
func OpenScratch() (*Database, error) {
	dir, err := os.MkdirTemp("", "folio2-pocketbase-*")
	if err != nil {
...
// folio2 schema exists in it. app is expected to be the single instance
// shared by the whole CLI (see cmd/folio2/main.go); its data directory is
// controlled by PocketBase's standard "--dir" flag, not by folio2 itself.
